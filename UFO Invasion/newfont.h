...
//          float color1r = points[f.v[0]][1]*20;
//          float color1g = points[f.v[0]][1]*10;
//          float color1b = points[f.v[0]][1]*.10;
          
          {
          float alpha = points[f.v[0]][1]*10;
          if (alpha < 0) alpha=0;
          if (alpha > 1) alpha=1;
          
          float botR = 1, botG = 1, botB = 0;
          float topR = 1, topG = 0, topB = 0;
          
          float color1r = alpha*topR + (1-alpha)*botR;
           float color1g = alpha*topG + (1-alpha)*botG;
           float color1b = alpha*topB + (1-alpha)*botB;
          
          glColor3f(color1r, color1g, color1b);
          }
          
          
   //       glColor3f(points[f.v[0]][1], <#GLfloat green#>, <#GLfloat blue#>)
	glNormal3f (normals[f.n[0]][0], normals[f.n[0]][1], normals[f.n[0]][2]);
	glVertex3f (points[f.v[0]][0], points[f.v[0]][1], points[f.v[0]][2]);
          
          
          
         /* float color2r = points[f.v[1]][1]*20;
          float color2g = points[f.v[1]][1]*10;
          float color2b = points[f.v[1]][1]*.10;
          
                  glColor3f(color2r, color2g, color2b);*/
          
          
          {
              float alpha = points[f.v[1]][1]*10;
              if (alpha < 0) alpha=0;
              if (alpha > 1) alpha=1;
	...
	glVertex3f (points[f.v[1]][0], points[f.v[1]][1], points[f.v[1]][2]);
         ...
         /* 
          float color3r = points[f.v[2]][1]*20;
          float color3g = points[f.v[2]][1]*10;
          float color3b = points[f.v[2]][1]*.10;
          
          glColor3f(color3r, color3g, color3b);*/
          
          {
              float alpha = points[f.v[2]][1]*10;
              if (alpha < 0) alpha=0;
              if (alpha > 1) alpha=1;
